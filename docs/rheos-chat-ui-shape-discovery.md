...
| S2 | `KanbanEvent` / `KanbanEnvelope` ad-hoc event maps in `domain/events.cljs` | **Canonical `envelope-schema`** with `:event/id :event/type :event/time :causal/root :session/id :turn/id :delivery/mode :payload` + `validate-envelope` and auto-defaults (seq, timestamps, causal ids) | `event-ledger/src/promethean/event_ledger/schema.cljs:18`; mirrored in `protocols/.../openplanner_protocols.cljs` | Rheos already writes to this ledger — it should emit **conformant envelopes**, domain data under `:payload`. |
