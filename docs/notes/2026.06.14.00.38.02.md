...
```
Because you already share `~/devel` across your Ubuntu machines, a natural fit is to make `~/devel/worlds/<name>/` hold the reproducible client and `~/devel/projections/<world>/<task>/` hold worktrees or sparse task views.  That keeps the stable composition separate from the ephemeral task lens, which is exactly the distinction your “worlds / projections” language is trying to preserve.[^1_1]
```
