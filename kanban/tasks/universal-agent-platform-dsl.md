...
title: "Universal Agent Platform DSL — Boundary + Muse Host Translation"
status: "in_progress"
priority: "P0"
labels: ["cljs", "opencode", "dsl", "boundary", "extensions", "muse", "katamorph"]
created_at: "2026-07-11T13:00:00Z"
category: "tasks"
write-id: "1785363543475-0.pjng2uf96mjgmttbfj"
---

# Universal Agent Platform DSL — Boundary + Muse Host Translation

Build the ημ data-first agent platform DSL with strict JS boundaries in
`packages/extensions`, while Muse owns host-neutral assembly and host-target projection.
...
- Host plugin/tool modules are generated by Muse target projections, not hand-written TS/JS.
...
## μ5 — Host translation in Muse

- `eta-mu.platform.target.opencode` never existed in `packages/extensions`; do not
  implement or restore it there.
- Muse consumes Katamorph declarations, resolves a target-neutral agent catalog, and
  projects that catalog into OpenCode, Claude, MCP, Sol, and future host adapters.
- Knoxx remains out of scope.

## Historical ημ config sketch

The following notes predate the Muse ownership decision and are retained as historical
input, not current acceptance criteria:
...
- `pnpm -C packages/extensions test` passes for the μ0–μ4 boundary, result, registry,
  and interpreter surface that actually exists there.
- `pnpm -C packages/extensions lint:kondo` reports zero warnings.
- No JS interop in `law`, `domain`, `shape`, `application`, or `infra` namespaces
  (kondo-enforced).
- Muse resolves portable provider, model-family, model, default-model, and agent
  references into a target-neutral catalog before any host is selected.
- Muse proves at least one OpenCode projection from that catalog, and rejects unknown or
  ambiguous references before rendering.
- No acceptance criterion requires `eta-mu.platform.target.opencode` in
  `packages/extensions`; Knoxx remains out of scope.

## References

- `packages/extensions/lib/eta_mu/opencode.cljs` (historical runtime adapter; not the owner of host translation)
...
- `spaces/muse/docs/design/contract-ownership-and-host-translation.md`
