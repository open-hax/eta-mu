...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@open-hax/eta-mu-ai`, `@open-hax/eta-mu-agent-core`, `@open-hax/eta-mu-coding-agent`, `@open-hax/eta-mu-tui`, `typebox`.
