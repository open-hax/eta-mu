...
(defmacro on-session-lifecycle
  ...
  [_ext-name state-sym & {:keys [on-start on-shutdown on-switch reset-fn _status-fn]}]
  `(do
     (em/on "session_start"
       :handler (fn [~'event ~'ctx]
                 (let [~state-sym (get-state)]
...
(defmacro on-turn-lifecycle
  ...
  [_ext-name state-sym & {:keys [on-turn-start on-message-end on-agent-end]}]
  `(do
     (em/on "turn_start"
       :handler (fn [~'event ~'ctx]
                 (let [~state-sym (get-state)
...
(defmacro on-tool-lifecycle
  ...
  [_ext-name state-sym & {:keys [on-start on-end _track-tools]}]
  (let [_track-tools-vec (or _track-tools :all)]
    `(do
