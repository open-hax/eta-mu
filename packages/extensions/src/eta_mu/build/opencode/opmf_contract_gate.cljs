(ns ^{:clj-kondo/ignore [:promise-chain/prefer-async-workflow]} eta-mu.build.opencode.opmf-contract-gate
  "OpenCode build adapter for the eta-mu OPMF contract gate.

  The promise chains in this file are intentional: OpenCode chat/text hooks
  receive a mutable output object and are expected to register side effects
  rather than return async values, so the adapter uses `.then`/`.catch` to
  preserve that synchronous hook contract."
