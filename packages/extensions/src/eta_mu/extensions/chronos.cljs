...
(defn ^:async chronos-api
  ...
  ([endpoint opts]
   (let [url (str CHRONOS-URL "/api" endpoint)
         ...
         resp (await (js/fetch url merged))]
     (if (.-ok resp)
       (await (.json resp))
       (throw (js/Error. (str "Chronos API error: " (.-status resp))))))))
...
(defn ^:async handle-status []
  (let [status (await (chronos-api "/agent/status"))
        active (js/Array.from (aget status "active"))
...
(defn ^:async handle-stop []
  (let [result (await (chronos-api "/sessions/stop-all" #js {:method "POST"}))]
...
(defn ^:async handle-list [params]
  (let [limit (or (aget params "limit") 10)
        sessions (await (chronos-api (str "/sessions?limit=" limit)))]
...
(defn ^:async handle-project-create [params]
  (if-not (aget params "project")
    (make-text-result "Error: project parameter required.")
    (let [result (await (chronos-api "/projects"
                                     #js {:method "POST"
...
(defn ^:async handle-project-list []
  (let [projects (await (chronos-api "/projects"))]
