(ns open-hax.sol.domain.contracts.resolve
  (:require [clojure.string :as str]
            [open-hax.sol.domain.agent.agent-templates :as templates]
            [open-hax.sol.domain.contracts.loader :as loader]
            [open-hax.sol.domain.contracts.roles :as roles]
            [open-hax.sol.domain.contracts.sources :as sources]))
...
(defn- normalize-tool-id [tool-id]
  (some-> tool-id str str/trim not-empty))

(defn- agent-role-claims [contract]
  (vec (or (get-in contract [:actor :roles])
           (:actor/roles contract)
           [])))

(defn- normalized-contract-actors [contract]
  (vec (or (:contract/actors contract)
           (when-let [actor (:contract/actor contract)] [actor])
           [])))

(defn- actor-claims->wire [actors]
  (vec actors))
...
(defn- normalize-agent-contract [contract]
  contract)
...
(defn- role-context
  ...
  (let [contract-role-slugs (->> (agent-role-claims contract)
                                 (map keywordish->role-slug)
                                 (remove nil?)
                                 distinct
                                 vec)
...
(defn- resolved-agent-map
  ...
  {:id (:id record)
   ...
   :contract-actor-ids (actor-claims->wire contract-actors)
  ...
  ([config contract-id actor-id]
   (when-let [id (some-> contract-id str str/trim not-empty)]
     (let [record (loader/find-contract-record-sync config "agents" id)
           ...
           contract (some-> contract0 normalize-agent-contract)]
       (when contract
         (let [enabled? (not (false? (:enabled contract)))
               contract-actors (normalized-contract-actors contract)
               requested-actor-id (some-> actor-id str str/trim not-empty)
               allowed-for-request? (or (nil? requested-actor-id)
                                        (actor-allowed? contract-actors requested-actor-id))
               effective-actor-id (effective-actor-id contract-actors requested-actor-id (default-actor-id config))]
