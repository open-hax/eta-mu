...
(defn write-file-sync!
  "Synchronous file write. Creates parent dirs recursively."
  [p text]
  (.mkdirSync node-fs (.dirname path (str p)) #js {:recursive true})
  (.writeFileSync node-fs (str p) (str text) "utf8"))
