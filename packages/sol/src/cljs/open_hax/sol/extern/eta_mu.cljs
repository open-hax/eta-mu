(ns open-hax.sol.extern.eta-mu
  ...
  (:require [clojure.string :as str]
            ...
            [open-hax.sol.shape.agent :refer [IAgentSession]]))
...
(defrecord EtaMuSession [^js raw]
  ...
  (set-active-tools!   [_ tool-names] (.setActiveToolsByName raw (clj->js tool-names)))
  (abort!              [_]   (.abort raw)))
...
(defn ^:async create-session!
  ...
  (let [create-agent-session (create-agent-session-fn)
    ...
    {:session (wrap-eta-mu-session (aget created "session") hook)
     :extensions-result (aget created "extensionsResult")
     :model-fallback-message (aget created "modelFallbackMessage")}))
