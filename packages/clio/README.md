...
`verification.edn`, when present on the branch, is generated by the one-shot CI
verifier only after recording the actual frozen-install, lint, NBB/Shadow, and
root-gate outcomes for the commit named inside the receipt.
