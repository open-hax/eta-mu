...
function pushPanel(panels: PanelName[], panel: PanelName): void {
...
function createCandidate(
  context: EtaMuPlanningContext,
  index: number,
  candidate: Omit<MuCandidate, "id">,
): MuCandidate {
...
function candidatePriority(kind: MuCandidateKind): number {
  switch (kind) {
    case "defer":
      return 1.0;
...
export function selectPanelsFromContext(
  ...
  const panels: PanelName[] = ["field", "movement"];

  if (
    context.belief.reviewDebt >= 0.4 ||
...
export function rankCheapMuCandidates(
  ...
  const context = normalizeContext(contextInput);
  const candidates: MuCandidate[] = [];

  if (context.failingChecks.length > 0) {
    candidates.push(
      createCandidate(context, candidates.length, {
        kind: "comment",
  ...
  return candidates
    ...
    .sort(
      (left, right) =>
        right.confidence + candidatePriority(right.kind) -
